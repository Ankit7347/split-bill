...
export default function BalanceTable({ room, expenses, memberMap }) {
  ...
  const monthKey =
    selectedMonth || (sortedMonthKeys.length > 0 ? sortedMonthKeys[0] : null);
  ...
  Object.keys(netBalances).forEach((id) => {
    if (netBalances[id] > 0) creditors.push({ id, amount: netBalances[id] });
      ...
      debtors.push({ id, amount: -netBalances[id] });
  });
  ...
  return (
    <div className="mt-6 space-y-4">
      ...
      <div className="mb-4">
        <label className="font-medium mr-2 text-gray-900 dark:text-gray-100">
          Select Month:
        </label>
        <select
      ...
      {monthKey && (
        <div className="border rounded shadow p-4 bg-white dark:bg-gray-900 dark:border-gray-700">
          <h2 className="font-bold mb-2 text-lg text-gray-900 dark:text-gray-100">
            Monthly Summary
          </h2>
          ...
          <table className="w-full border text-sm mb-2">
            ...
            <tbody>
              {room.members.map((m) => {
                ...
                return (
                  <tr key={m.uuid}>
                    ...
                    <td className="border px-2 py-1 text-right text-gray-900 dark:text-gray-100">
                      {spent.toFixed(2)}
                    </td>
                  </tr>
                );
              ...
              {/* Total row */}
              <tr>
                <td className="border px-2 py-1 font-bold text-gray-900 dark:text-gray-100">
                  Total
                </td>
                <td className="border px-2 py-1 text-right font-bold text-gray-900 dark:text-gray-100">
              ...
              <tr>
                ...
                <td className="border px-2 py-1 text-right font-bold text-gray-900 dark:text-gray-100">
                  {(totalMonthlyExpense / room.members.length).toFixed(2)}
                </td>
              </tr>
            </tbody>
          </table>
            ...
            <tbody>
              {room.members.map((m) => (
                <tr key={m.uuid}>
                  ...
                  <td
                    className={`border px-2 py-1 text-right text-gray-900 dark:text-gray-100 ${
      ...
      <div className="border rounded shadow p-4 bg-white dark:bg-gray-900 dark:border-gray-700">
        ...
        <table className="w-full border text-sm">
          <thead className="bg-gray-100 dark:bg-gray-800">
            <tr>
              <th className="border px-2 py-1 text-left text-gray-900 dark:text-gray-100">
                Member
              </th>
              <th className="border px-2 py-1 text-right text-gray-900 dark:text-gray-100">
                Balance (₹)
              </th>
            </tr>
          </thead>
          <tbody>
            {room.members.map((m) => (
              <tr key={m.uuid}>
                <td className="border px-2 py-1 dark:text-gray-100">
                  {m.name}
                </td>
                <td
